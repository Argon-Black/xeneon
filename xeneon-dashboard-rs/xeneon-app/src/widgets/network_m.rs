...
//! half-column (same height as SQ, double the width). This is the last
//! of the SSX/SX/S/SQ/M variants from the mockup shown to the user before
//! any of this was built (excluding L, which was explicitly out of scope
//! from the start).
//!
//! A near-duplicate of `network_sq.rs` - same interface Auto/pin +
//! rename settings, same icon+name/down/up color and icon+name size
//! settings, same Wi-Fi/Ethernet icon switching and machine-wide VPN
//! badge, same bundled-SVG icon rendering and Cairo graph - for all the
//! reasons that module's own doc comment already explains (also: same
//! reasoning as `cpu_temp.rs`/`temp_gauge.rs`, or `network_sx.rs`/
//! `network_s.rs`, choosing duplication over a shared abstraction for a
//! settings/appearance UI this size). Two differences from SQ: the rate
//! line's ellipsize cap is a bit more generous (more width to spare), and
//! the history buffer holds more samples (three minutes instead of two -
//! there's a lot more horizontal room to show a longer trend in).
//! Reuses `network.rs`'s `read_interfaces`/`default_interface`/
//! `format_rate_fixed`/`is_wireless`/`vpn_active`, same as SQ does.
...
/// Same blue/coral pairing as `network.rs`'s SSX card and `network_sq.rs`'s
/// SQ card (see `network::Direction::color_hex`) - just the *default* now,
/// since both are user-editable here (see `set_down_color`/`set_up_color`).
...
/// More generous than SQ's cap - this card is roughly twice as wide, while
/// the rate line's real content doesn't grow at all (it never included
/// the name to begin with). Still only a safety net for pathological
/// cases; normal content never gets close.
const MAX_VALUES_WIDTH_CHARS: i32 = 34;

/// Three minutes of history (SQ keeps two) - this card has roughly twice
/// SQ's width to spread the same number of samples across, so a longer
/// window fills it out rather than leaving the trace looking sparse.
...
/// same reasoning as `audio.rs`'s `EMPTY_STATE_ICON_RASTER_PX`.
...
fn ensure_css_installed() {
    INSTALL_CSS.call_once(|| {
        ...
        css.load_from_string(&format!(
            ".xeneon-network-m-values {{ font-size: {VALUES_FONT_PX}px; color: rgba(255, 255, 255, 0.75); }}\n\
             .xeneon-network-m-vpn-badge {{ background-color: rgba(93, 202, 165, 0.15); \
             border-radius: 13px; padding: 5px 12px; }}\n\
             .xeneon-network-m-vpn-badge-label {{ font-size: {VPN_BADGE_FONT_PX}px; font-weight: 700; \
             color: {VPN_COLOR_HEX}; }}"
        ));
...
// never bleeds into another's. Same registry pattern as `network_sq.rs`'s
// `NAME_CSS` (see `appearance_css::CssRuleRegistry`'s own doc comment).
thread_local! {
...
/// shape as `network_sq::NetworkSqState` - see that struct's doc comment.
struct NetworkMState {
    css_class: String,
...
impl NetworkMState {
    ...
    /// `network_sq::NetworkSqState::reset`/`ClockState::reset`; called
    /// from the appearance popover's reset button via `WidgetInstance::
    /// on_reset`, wired up in `spawn`/`restore` below.
    fn reset(&self) {
    ...
    /// change - same call sites as `network_sq::NetworkSqState::refresh`.
    fn refresh(&self) {
...
fn draw_graph(state: &NetworkMState, cr: &gtk::cairo::Context, width: i32, height: i32) {
...
fn build_content() -> (Rc<NetworkMState>, gtk::Widget) {
    ensure_css_installed();

    static NEXT_ID: AtomicU64 = AtomicU64::new(0);
    let css_class = format!("xeneon-networkm-{}", NEXT_ID.fetch_add(1, Ordering::Relaxed));
    ...
    // No `set_halign(Start)` here - left at the default `Fill` so this
    // row spans the full card width, which the spacer below needs to
    // push the VPN badge to the far right edge rather than right up
    // against the name.
    ...
    vpn_badge.add_css_class("xeneon-network-m-vpn-badge");
    ...
    vpn_badge_label.add_css_class("xeneon-network-m-vpn-badge-label");
    ...
    values_label.add_css_class("xeneon-network-m-values");
    ...
    let state = Rc::new(NetworkMState {
        css_class,
        icon_image,
        name_label,
        vpn_badge,
...
/// rename field once one is pinned, then the four appearance settings
/// this card adds on top - icon+name color, icon+name size, download
/// color, upload color - identical to `network_sq::build_settings`.
fn build_settings(state: Rc<NetworkMState>) -> (gtk::Widget, Box<dyn Fn()>) {
    ...
    // `network_sq.rs`/`clock.rs`'s own `resync` - see those for why
    // ordering matters (every value read out before any control is
    // touched).
